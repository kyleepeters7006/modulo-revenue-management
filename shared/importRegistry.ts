...
export const IMPORT_DATASETS: DatasetDefinition[] = [
  ...
  },
  {
    id: "move_ins_outs",
    name: "Move Ins & Outs",
    description: "Admission and discharge events from the Move Ins or Move Outs export report. One direction per file — upload move-ins and move-outs as separate files. Powers move-in/out counts across the dashboard (Reference Data, rule performance, pricing impact). Use the specialized uploader on the Uploads tab.",
    targetTable: "move_in_out_events",
    periodField: null,
    filenamePeriodPattern: "(20\\d{2})[-_]?(0[1-9]|1[0-2])",
    replacesPeriod: false,
    fields: [
      { key: "date", label: "Date", type: "date", required: true, format: "M/D/YYYY or YYYY-MM-DD", description: "Event date", sample: "7/3/2026", aliases: ["Event Date"] },
      { key: "campus", label: "Campus", type: "string", required: true, description: "Campus name — must match Location Data", sample: "Anderson - 112", aliases: ["Location", "Facility"] },
      { key: "department", label: "Department", type: "string", required: false, description: "Service line code (HC, AL, AL/MC, VIL, SL). Preferred over Service Line when both are present.", sample: "AL", aliases: ["Dept", "Service Line Code"] },
      { key: "serviceLine", label: "Service Line", type: "string", required: false, description: "Text fallback when Department is absent: Health Center, Assisted Living, Memory Care, Skilled Nursing, Independent Living, Villas.", sample: "Assisted Living" },
      { key: "roomBed", label: "Room/Bed", type: "string", required: false, description: "Room and bed identifier, e.g. 204/A. The portion after the slash is read as the bed position.", sample: "204/A", aliases: ["Room", "Bed", "Room Number"] },
      { key: "lastName", label: "Last Name", type: "string", required: false, description: "Resident last name. Used with date, campus and room to de-duplicate rows within a file.", sample: "Sample", aliases: ["Resident Last Name"] },
      { key: "payerName", label: "Payer Name", type: "string", required: false, description: "Payment source (e.g. Private Pay, Medicaid, Medicare A, Managed Care).", sample: "Private Pay", aliases: ["Payor", "Payor Name", "Payer"] },
      { key: "moveEvent", label: "Move Event", type: "enum", required: false, allowedValues: ["Admission", "Return", "Transfer"], description: "Event type. Only Admission rows are counted as move-ins.", sample: "Admission" },
      { key: "moveCategory", label: "Move Category", type: "string", required: false, description: "Free-text category. For move-outs, must be exactly 'Discharge - Return Not Anticipated' to be counted as a permanent discharge.", sample: "New Admission", aliases: ["Category"] },
      { key: "moveIns", label: "Move Ins", type: "integer", required: false, description: "Marker column for move-in files. Its value is not used — only its presence sets the direction as move-ins. Replace with 'Move Outs' column for discharge files.", sample: "1" },
    ],
  },
  {
    id: "campus_location",
