...
- Mutations that create or replace annual in-house plan proposals must also invalidate the server-side Reference Data cache after commit; browser-query invalidation alone can serve the pre-plan payload until the ten-minute TTL expires.
