...
**How to apply:** Keep below-target warnings informational and preserve infeasibility details in the saved plan. Submit every calculated service line. Block only when the exact raw input snapshot captured at calculation differs from current inputs; returned solver assumptions are normalized and can create false stale warnings. Persist that snapshot with restored calculations, with a per-line identity when a multi-line result can later be opened one line at a time.