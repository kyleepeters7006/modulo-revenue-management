...
**How to apply:** Keep future user-management, invitation, recovery, and support flows tenant-scoped. Preserve the Trilogy-only, read-only activity-review exception and keep each tenant visibly partitioned. Store only reset-token digests and revoke sessions after credential changes. A user-facing deletion must be a marked soft deletion that disappears from account management while preserving historical attribution; prevent self-deletion and removal of the last active tenant administrator.