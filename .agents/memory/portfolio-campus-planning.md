...
**How to apply:** Pass the portfolio input snapshot explicitly into the campus fan-out instead of resolving existing campus overrides. Because report snapshots intentionally omit resident rows, selecting a restored campus must refresh that exact calculation once live inputs and tier policies hydrate; never present the empty snapshot array as 0 residents. Portfolio scatterplots must show one dot per campus/service-line calculation. Full live plans can derive the campus weighted result from their resident recommendations; restored compact portfolio reports must read the saved campus calculations produced by the fan-out rather than collapsing to one portfolio point per service line. A later explicit campus save/rerun creates a more-specific override for that campus only. Never let an older fan-out overwrite a newer report.