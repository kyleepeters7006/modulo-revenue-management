...
- Every AI suggestion must name exactly one canonical room type; enforce this during generation and again during acceptance. **Why:** operators with only a few buildings manage pricing at product detail, and a broad service-line rule hides materially different room economics.
...
- Size Street Rate suggestions using signed, care-adjusted competitor variance for that exact room type where available: below market supports more upward pressure; above market calls for restraint unless occupancy proves pricing power. Never use competitor data as a hard cap/floor or sole sizing input because survey entry errors are possible.
