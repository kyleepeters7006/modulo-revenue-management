...
**Room-type keys from the event feed are only trustworthy at service-line level.** The export feed can leave `room_type` null for every event. For a per-room-type display, first match `room_name` to the spot-month rent roll's `room_number`, take the rent-roll service line and room type, then apply `room_type_groupings`. This also maps event-feed `IL` to rent-roll `VIL`. Aggregate to service line when no room-level resolution is possible; otherwise per-room-type figures print phantom zeros beside real events.
