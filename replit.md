...
- **Mixed Rate Storage Model (November 2025)**: HC and HC/MC rates are stored as DAILY rates in database (as imported from Trilogy CSV), while AL, SL, VIL, AL/MC rates are stored as MONTHLY rates. Frontend displays rates in their stored format ($339/day for HC, $5,000/month for AL). Backend revenue calculations convert all rates to monthly using 30.44-day month standard for accurate annualization.
...
- **Competitive Survey Import & Auto-Matching**: Imports wide-format Excel competitive survey data (9,727+ records) and automatically matches competitors to rent roll units by location + service line + room type. System processes all units (17,216) in batches of 500 using a persistent job queue that survives server restarts. Matching runs automatically after both competitive survey import AND rent roll upload via the job-based system.
- **Job-Based Competitor Rate Matching (November 2025)**: Persistent `competitor_rate_jobs` table tracks job progress (total units, processed, updated, skipped, errors). Jobs use `lastProcessedId` for deterministic pagination and resume from the exact position after server restarts. API endpoints: `POST /api/competitor-rates/job/start`, `GET /api/competitor-rates/job/:jobId`, `GET /api/competitor-rates/jobs/:uploadMonth`. Server startup automatically resumes interrupted jobs.
