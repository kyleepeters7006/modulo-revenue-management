  ...
  app.get("/api/pricing-controls/commentary", async (req: any, res) => {
    try {
      ...
      if (regions.length > 0) {
        ...
        rrFilters.push(`l.region = ANY($${rrParams.length})`);
      }
      if (divisions.length > 0) {
        rrParams.push(divisions);
        rrFilters.push(`l.division = ANY($${rrParams.length})`);
      }
      if (serviceLine && serviceLine !== 'All') {
        rrParams.push(serviceLine);
      ...
      const [snapshotBySLRes, trendRes, rulesRes, rohRes, competitorRes, rohTrendRes] = await Promise.all([
        ...
        pool.query(`
          ...
          WHERE rrd.client_id = $1
            ...
            ${rrWhere}
          GROUP BY rrd.upload_month, rrd.service_line
      ...
      // Rule detail for GPT
      const ruleDetails = ruleRows.map((r: any) => {
        const action = typeof r.action === 'string' ? JSON.parse(r.action) : (r.action || {});
      ...
      const filterContext = [
        locations.length > 0 ? `locations: ${locations.join(', ')}` : null,
        regions.length > 0 ? `regions: ${regions.join(', ')}` : null,
        divisions.length > 0 ? `divisions: ${divisions.join(', ')}` : null,
...
` : '';
      ...
      const recommendationSystem = isUnfiltered
        ? `You are a senior revenue management strategist for senior living portfolios. Write a portfolio-level strategy recommendation as 3-5 short bullet action items (each on its own line, starting with "• "). Cover, where the data supports it: (1) same-store vs development/acquisition class performance, (2) regional or division trends, (3) service-line trends, and (4) specific room-type observations. Every bullet must be a concrete action grounded in a specific number from the data. Use **double asterisks** around the key action and the most important number in each bullet. No preamble, no closing sentence — bullets only. IMPORTANT: Always refer to the VIL service line as "Patio Homes" — never use "VIL". IMPORTANT: Do NOT mention or flag rules with zero execution counts — an exec count of 0 simply means the current month's rent roll has not yet been uploaded and is NOT a system problem; omit any reference to execution counts entirely.`
        : `You are a senior revenue management strategist for senior living portfolios. Respond with exactly ONE concise sentence — a concrete, actionable pricing strategy recommendation grounded in the data provided. Use **double asterisks** around the key action and the most important number. IMPORTANT: Always refer to the VIL service line as "Patio Homes" — never use "VIL". IMPORTANT: Do NOT mention or flag rules with zero execution counts — an exec count of 0 simply means the current month's rent roll has not yet been uploaded and is NOT a system problem.`;
...
3. "rulesSummary": One sentence describing the overall pricing strategy implied by the active rules (e.g., aggressive rate-lift in AL, defensive discounting in HC, etc.). Bold the strategy type and service lines.

4. "rules": Array of objects, one per active rule, each with:
   - "name": the rule name exactly as given
   - "strategy": one short sentence describing what this rule does strategically (not just repeating the description — explain the WHY and IMPACT). Bold the key metric or outcome.

Return ONLY valid JSON, no markdown fences:
{"summary":"...","pricingTrend":"...","rulesSummary":"...","rules":[{"name":"...","strategy":"..."}]}`,
          },
        ],
      });
  ...
  app.get("/api/rule-performance", async (req, res) => {
    try {
      ...
      const revRes = await pool.query<{ upload_month: string; location: string; service_line: string; room_type: string; revenue: number; occ_count: number; occ_all: number; total_count: number }>(`
      ...
      const revSeries = new Map<string, { month: string; revenue: number; occ: number; occAll: number; total: number }[]>();
      for (const r of revRes.rows) {
        const k = `${r.location}|${r.service_line}|${r.room_type}`;
        if (!revSeries.has(k)) revSeries.set(k, []);
        revSeries.get(k)!.push({ month: r.upload_month, revenue: Number(r.revenue), occ: Number(r.occ_count), occAll: Number(r.occ_all), total: Number(r.total_count) });
      }

      // ---- T3 move-in event series -------------------------------------------
      ...
      const t3MoveInsFor = (groupKey: string, appliedMonth: string): { before: number; after: number } | null => {
        const snaps = revSeries.get(groupKey);
        if (!snaps || snaps.length === 0) return null;
        const beforeMonths = snaps.filter(p => p.month < appliedMonth).slice(-3).map(p => p.month);
        const afterMonths = snaps.filter(p => p.month >= appliedMonth).slice(0, 3).map(p => p.month);
      ...
      // mixes the private-pay-only revenue occ count with impacted-unit counts).
      const t3OccPctBefore = (groupKey: string, appliedMonth: string): number | null => {
        const snaps = revSeries.get(groupKey);
        if (!snaps || snaps.length === 0) return null;
        const before = snaps.filter(p => p.month < appliedMonth).slice(-3);
        const occ = before.reduce((s, p) => s + p.occAll, 0);
      ...
      for (const [rgKey, rg] of Array.from(ruleGroupWeight.entries())) {
        ...
        if (mi) { dc.miBefore += mi.before * frac; dc.miAfter += mi.after * frac; dc.hasMi = true; }
        if (occPct != null) { const w = t3.occBefore > 0 ? t3.occBefore : 1; dc.occPctNum += occPct * w; dc.occPctDen += w; }
      ...
      const finish = (a: Agg, c: Calc | undefined, useT3 = false) => {
        ...
        return {
          ...
          calc: has ? {
            ...
            moveInsT3Before: c!.hasMi ? Math.round(c!.miBefore * 10) / 10 : null,
            moveInsT3After: c!.hasMi ? Math.round(c!.miAfter * 10) / 10 : null,
            occPctBefore: c!.occPctDen > 0 ? Math.round((c!.occPctNum / c!.occPctDen) * 10) / 10 : null,
          } : null,
        };
      };
  ...
  app.get("/api/reference-data", async (req, res) => {
    try {
      ...
      // ── Physical room vacancy from history (authoritative, excludes B-bed double-counting) ──
    ...
    try {
      ...
      const rulePreviewMap = new Map<string, number>();
      {
        ...
        if (activeRules2.length) {
          ...
          const campOcc2 = new Map<string, number>();
          campT.forEach((v, k) => campOcc2.set(k, v.total ? v.occ / v.total : 0));
          const slOcc2 = new Map<string, number>();
          slT.forEach((v, k) => slOcc2.set(k, v.total ? v.occ / v.total : 0));
          // Override with authoritative occupancy history (anchored to the
          ...
          // rule triggers evaluate against identical occupancy metrics.
          {
            ...
            campAcc.forEach((e, k) => { if (e.avail > 0) campOcc2.set(k, e.occ / e.avail); });
          }
