...
export default function PricingWeights({ locationId, serviceLine }: PricingWeightsProps = {}) {
  ...
  useEffect(() => {
    if (weightsResponse && typeof weightsResponse === 'object' && 'weights' in weightsResponse && weightsResponse.weights) {
      ...
      const loadedWeights = {
        occupancyPressure: apiWeights.occupancy_pressure ?? 44,
        daysVacantDecay: apiWeights.days_vacant_decay ?? 10,
        seasonality: apiWeights.seasonality ?? 10,
        competitorRates: apiWeights.competitor_rates ?? 12,
        stockMarket: apiWeights.stock_market ?? 10,
        inquiryTourVolume: apiWeights.inquiry_tour_volume ?? 14,
      };
  ...
  return (
    <div className="dashboard-card mb-8">
      ...
      <div className="flex items-center justify-between p-4 mb-4 bg-gray-50 dark:bg-gray-950/20 rounded-lg border border-gray-200 dark:border-gray-700">
        <div className="flex-1">
