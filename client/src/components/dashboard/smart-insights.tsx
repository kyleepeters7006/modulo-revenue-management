...
export default function SmartInsights() {
  const [suggestions, setSuggestions] = useState("Smart insights will appear here after analysis...");
  ...
  const smartSuggestMutation = useMutation({
    mutationFn: async () => {
      return apiRequest('POST', '/api/smart/suggest');
    },
  ...
  const trainModelMutation = useMutation({
    mutationFn: async (file?: File) => {
      if (file) {
        ...
        return apiRequest('POST', '/api/smart/train', formData);
      } else {
        return apiRequest('POST', '/api/smart/train');
      }
    },
    onSuccess: async (response) => {
      const data = await response.json();
  ...
  return (
    <div className="dashboard-card mb-8">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-semibold text-[var(--dashboard-text)]" data-testid="text-smart-insights-title">
            Smart Insights & Analytics
          </h3>
          <p className="text-sm text-[var(--dashboard-muted)]">
            Advanced analytics and predictive insights for pricing optimization
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Data-driven insights and suggestions</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            >
              {smartSuggestMutation.isPending ? "Analyzing..." : "Generate Smart Insights"}
            </Button>
